...
from ml_pipeline.src.constants import ARTIFACT_FILENAMES, RISK_THRESHOLDS
...
class ChurnPredictor:
    ...
    def _predict_probability(self, transformed_data: pd.DataFrame) -> pd.Series:
        ...
        if hasattr(self.model, "predict_proba"):
            return pd.Series(
                self.model.predict_proba(transformed_data)[:, 1],
        ...
        return pd.Series(
            self.model.predict(transformed_data),
            index=transformed_data.index,
        )
