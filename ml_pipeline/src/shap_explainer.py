...
from ml_pipeline.src.config import MODELS_DIR, PROCESSED_DATA_DIR, REPORTS_DIR, SCREENSHOTS_DIR
...
class SHAPExplainer:
    ...
    def _load_preprocessing_pipeline(self) -> Any:
        pipeline_path = self.models_dir / ARTIFACT_FILENAMES["preprocessing_pipeline"]
        if not pipeline_path.exists():
            raise FileNotFoundError(f"Preprocessing pipeline not found: {pipeline_path}")
        return joblib.load(pipeline_path)

    def explain_single(self, customer_data: dict) -> dict:
        if shap is None:
            raise ImportError("shap is not installed")

        if not hasattr(self, "pipeline"):
            self.pipeline = self._load_preprocessing_pipeline()
        ...
        row_df = pd.DataFrame(transformed, columns=self.feature_columns, index=raw_df.index)
        ...
        if hasattr(self.model, "predict_proba"):
            prediction_probability = float(self.model.predict_proba(row_df)[0, 1])
        else:
            prediction_probability = float(self.model.predict(row_df)[0])
            ...
            pd.DataFrame(
                {
                    ...
                    "feature_value": row_df.iloc[0].values,
                }
            )
            .assign(abs_shap=lambda df: np.abs(df["shap_value"]))
